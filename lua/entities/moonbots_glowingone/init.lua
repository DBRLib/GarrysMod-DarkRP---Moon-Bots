...
function ENT:Initialize()
	...
	self:SetHealth( math.Rand(5000,7000) )
...
function ENT:HaveEnemy()
	...
	if ( self:GetEnemy() and IsValid( self:GetEnemy() ) ) then
		...
		if(self:GetRangeTo( self:GetEnemy():GetPos() )  == self.lastDistToEnemy) then
			...
			timer.Simple( 5, function() self.ensureDiffrentEnemy = nil end )
...
function ENT:OnContact( ent )
	...
	if ent:IsPlayer() || ent:IsNPC() then
		...
		dinfo:SetDamage( math.Rand(35,50) )
